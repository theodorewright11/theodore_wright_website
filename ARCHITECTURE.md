...
- `aiThemeImport.ts` — `buildThemesFromAI(project, raw, now)`: the inverse of `exportCorpusForAI`. Takes an AI thematic-analysis JSON (`{ themes: [{ reasoning, name, definition, justification, quotes: [{ text, source, role }] }] }`) and merges it into an existing project as new `Theme`s. Each quote is resolved — `source` (`[D{n}]`) → `documents[n-1]`, verbatim `text` → character offsets via `indexOf` — into a `ThemeUncodedHighlight` (theme evidence, no code). `reasoning`/`definition`/`justification` compose the theme's `description` markdown; `role` → highlight weight. Quotes that don't locate verbatim are returned in `unmatched` (surfaced to the user, never silently dropped). Triggered by the "import AI ↓" button in the Themes view header.
