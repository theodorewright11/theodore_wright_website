...
- `drive.ts` — the Drive sync layer. Same GIS browser-side OAuth pattern as `finance/sheets.ts` and `time-tracker/sheets.ts`, but Drive Files API instead of Sheets. Token in `sessionStorage` (`tw-qual-coding-google-token`, 60s expiry safety margin). Scope `https://www.googleapis.com/auth/drive.file` — file-level scope, *not* full Drive. All app-created files are tagged with `appProperties.tw_qual_coding=v1` so the listing query (`listAppFiles`) filters to only the app's own files. Functions: `loadStoredToken` / `signIn` / `signOut`; `listAppFiles(token, folderId?)`; `getFileContent<T>`; `createFile` (multipart upload — `multipart/related` with a `tw_qc_boundary_<random>` boundary, two parts: metadata JSON + content JSON); `updateFile` (media-only PATCH to `/upload/drive/v3/files/{id}?uploadType=media`, plus an optional metadata PATCH to rename); `deleteFile`. `DriveAuthError` (401/403) triggers token drop + a "Sign in again" surface.
...
**Drive sync mechanics**:

- One Drive file per `Project`. Filename `projectFileName(name, id)` = `<name-slug>.<id-prefix>.qcoding.json`. Filename is updated when the project is renamed (PATCH metadata before the content PUT).
- App-properties tag: every app-created file carries `appProperties: { tw_qual_coding: 'v1' }`. `listAppFiles` queries `appProperties has { key='tw_qual_coding' and value='v1' } and trashed=false`, optionally `'<folderId>' in parents` if `PUBLIC_QUAL_CODING_DRIVE_FOLDER_ID` is set.
- **Sync queue** (`pendingWrites`, `inflightWrites` refs in the root component): per-project, debounce 800ms then `runWrite`. If a write is already in-flight for that project, re-queue. `runWrite` reads the current project from localStorage (which is always fresh — the `saveState` useEffect flushes synchronously after each `setState`), then `createFile` if no `drive.fileId` yet, or `updateFile` otherwise. The returned file's `id` + `modifiedTime` is written back into the project's `drive` field via `updateProjectById`.
- **Pull lifecycle** (`pullAllFromDrive`): triggered on sign-in and on `window.focus`. Lists all app files, fetches each as JSON, coerces into `Project` (preserving the `drive` link from the file id + modifiedTime), then merges into local state — server projects replace local entries by id; local projects not seen in the server list are retained; local projects without a `drive.fileId` are queued for an immediate write so the next pull is symmetrical.
