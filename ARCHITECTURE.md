...
Reads are tolerant of column reorders (matched by header name via `rowsToObjects`). **Transaction dates are normalized to ISO on read** (`toIsoDate` in `compute.ts`) — a CSV appended straight into the sheet gets its date column auto-converted by Google Sheets to date-typed cells that render as `M/D/YYYY`, which the month math can't parse; normalization heals that (also handles serials). App-written rows stay ISO text (RAW writes). Writes use the canonical order — every write rewrites the header row too. A pull with an empty `categories` tab seeds `DEFAULT_CATEGORIES` into state and pushes it back up. Existing tabs (`Current Budget`, `Spending Log`, `Dashboard`) are not touched except for the one-shot read in `spendingLogImporter.ts`, which reads the legacy `Spending Log` tab to seed historical transactions (it takes the live `categories` to flag unknown ones).
...
- `ThemesView.tsx` — analyst-level grouping layer above codes. Hierarchical theme tree + a theme-detail card: separate **Definition** and **Reasoning** Markdown editors, linked annotations toggled core/supporting, uncoded highlights, an include-codes picker (`HierarchicalCodePicker`), and the rating card (five hand-rated axes — grounding + research-question-fit evaluative, interpretation level + AI-prior-novelty + analytical-novelty descriptive; the two novelty axes scored vs. the no-data baseline) plus a **Theme similarity** section (`Project.themeRelations`) that replaces the Independence score — link a theme to others via `related` (symmetric) or `subsumes` (directed), each with a 1–5 similarity. The sidebar header carries an **"import AI ↓"** button (loads an AI thematic-analysis JSON via `aiThemeImport.buildThemesFromAI` and merges themes + located quote spans into the active project) and a **"doc"** toggle that swaps the detail pane for `ThemesDocView` — every theme rendered as a depth-nested heading with definition, reasoning, supporting `[D{n}]`-tagged quotes, and the same inline rating control for read-and-score.
- `GradingView.tsx` — rubric-rating dashboard with three tabs: **Codes** (specificity 1–5), **Annotations** (accuracy 1–5, with a per-code drill-down), and **Themes** (five hand-rated axes — grounding / research question fit / interpretation level / AI prior novelty / analytical novelty; prevalence computed, independence via theme-similarity links). Designed for grading one project's coding against another (e.g. AI's vs the analyst's).
...
- `AuthBar.tsx` — the Drive sign-in pill in the TopBar. Visual states: `local only` (no env var), `Sign in to sync` (configured, no token), `signed-in` (email + colored dot — green idle / blue pulsing syncing / red error / grey offline), with a dropdown showing file count, last error, **"Sync now"** (flush pending writes + push the active project immediately, skipping the 800ms debounce), **"Refresh from Drive"** (pull), and Sign out.
...
- `driveSync.ts` — per-project sync orchestrator (separated from `drive.ts` so the REST helpers stay pure). Owns the folder-per-project layout. Exports `syncProjectToDrive(token, project, rootFolderId?)` (returns updated `DriveLink`), `pullProjectFromDrive(token, source)` (handles both new and legacy sources), and `deleteProjectFromDrive(token, drive)` (deletes the whole folder). **Stale-folder recovery**: before writing, it `fileExists`-checks `drive.folderId` (and the configured root folder); a folder deleted out from under the project is treated as missing and recreated, falling back to My Drive root if the configured root is gone — so a deleted Drive folder self-heals instead of 404-ing every write.
...
- **Incremental writes (hash-skip)**: `project.json` (canonical) is always written, but each derived export — `project.md` / `codebook.md` / `themes.md` / `corpus.md` and the whole `documents/` subtree — is only rewritten when its content hash differs from the one stored on `DriveLink.hashes` (set on the last successful sync). A rating/theme edit therefore uploads `project.json` + the 1–2 exports that actually changed and **skips the entire per-document tree walk + writes**, instead of re-uploading ~`N`+5 files every time. Hashes are preserved across focus-pulls (the merge keeps `local.drive.hashes`); a failed sync returns no `DriveLink`, so the next attempt recomputes and rewrites. First sync after the feature lands does one full rewrite to populate the hashes.
...
- **Pull lifecycle** (`pullAllFromDrive`): triggered on sign-in and on `window.focus`. Lists all app-tagged files (= all `project.json` files), fetches each, classifies as legacy-flat (parent is the root folder) or in-folder (parent is a project folder), stamps the appropriate `DriveLink` shape, and merges into local state. The merge runs **inside the functional `setState` against current in-memory `s.projects`** (not a `localStorage` snapshot) and is **timestamp-based** — a project whose in-memory `updated_at` is ≥ the server copy's wins, so a focus-pull firing in the gap between a `setState` and the `saveState` flush can't clobber a just-made edit (the bug that dropped rating selections). Local projects without `drive.folderId` are queued for an immediate write so migration happens at most once per machine.
