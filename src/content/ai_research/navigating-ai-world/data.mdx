---
title: 'Data'
description: 'Empirical pipeline that confronts the model''s six named fitting targets (Q1–Q6) with currently-published evidence. Two parameters supported by direct fits (gate τ from BCG-Randazzo modes; α from six pooled productivity studies). One supported qualitatively (relational dose-response from OpenAI-MIT N=981). One bounded from below (cumulative-atrophy speed λ — calculator-analogue ruled out). Two untestable from current data (κ population calibration, scalar-vs-vector identity allocation) — flagged as the load-bearing data gaps for the model''s defensive-side conclusions. Curated CSVs (downloadable) + Python pipeline + interactive findings panel.'
date: 2026-05-02
status: in-progress
refinementPass: 1
refinementLog:
...
Three verdicts hold cleanly. **Q3 (gate τ)**: BCG-Randazzo's three-mode distribution (27% self-automator at f·ρ ≈ 0.06; 73% cyborg-or-centaur at f·ρ ≈ 0.49 weighted centroid) implies a midpoint τ ≈ 0.25 — within 0.05 of the model's default 0.30 and well inside the modeled range. **Q6 (α)**: across six per-task productivity studies the implied α spans 0.24 (BCG productivity inside frontier) to 1.01 (Peng GitHub Copilot JS HTTP-server), median 0.45 — a 4.2× per-domain spread that confirms the model's claim that scalar α should be replaced by per-domain α. The model's default α=0.40 sits at the 37th percentile, a lower-middle anchor that under-represents coding and writing while over-representing realized economy-level effects. **Q2 (relational dose-response)**: the OpenAI-MIT N=981 RCT supports the piecewise shape — voluntary daily use predicts loneliness, dependence, and reduced in-person socialization regardless of modality, with low doses null-or-protective and a transition somewhere around 30 minutes/day. The slope ψ_R re-estimates to 0.0028 per minute (model default 0.003 — within rounding); β_R unchanged at 0.001. The catastrophic-loss mechanism (Replika ERP removal: mental-health Reddit posts went from 0.13% to 0.65%, χ²=11.04, p&lt;.001) is a separate failure mode the model does not encode.

**Q1 (λ atrophy speed)** is bounded from below only. Cross-sectional evidence (Gerlich 2025 N=666 cognitive-offloading correlation r ≈ −0.68; Stadler-Bannert-Sailer 2024 acute argument-quality drop; Kosmyna MIT 2025 reduced neural engagement; Bastani PNAS 2025 −17pp on unassisted retest; Ehsan 2026 year-long intuition rust) rules out the calculator-analogue tail (λ = 0) but cannot pin λ above the lower bound. Bastani scaled per offloading-rate × time gives λ ≈ 0.06–0.37/year depending on assumed window; Ehsan's slow-decay year-long anchor implies λ ≈ 0.10/year at moderate u. The model's default λ=0.06 sits at the lower edge of this range. The 2+ year longitudinal study that would actually pin λ does not yet exist — this remains the single largest unknown for the model's trajectory tab. **Q4 (scalar vs vector identity allocation T, B)** and **Q5 (κ population calibration)** are untestable from currently-collected data. ATUS gives time-use without identity weights; BPNSFS gives within-study κ without cross-population AI-exposure variation. Both await new survey instruments. Stage 5 should consume the model's defaults for these and treat them as user-tunable rather than fitted.

The pipeline is intentionally small: seven curated CSVs in `public/data/navigating-ai-world/` (every cell source-cited), one ~280-line Python script that produces every chart on this page, dependencies pandas + numpy. Total source corpus: 24 primary references, all web-verified against publication URLs. Two exploratory side-results worth noting separately from the Q1–Q6 confirmatory frame: the **apprenticeship-ladder break** is independently confirmed across the US payroll panel (Brynjolfsson-Chandar-Chen: 22-25-year-olds in highly AI-exposed occupations down 13%; software developers age 22-25 down 19.5% from late-2022 peak) and the global freelance market (Hui-Reshef-Zhou: −2% jobs and −5.2% earnings overall, with the top-performer paradox — high-earning freelancers hit hardest). And the **Anthropic Economic Index** shows a slow drift toward automation in consumer Claude.ai (57% → 51% augmentation share over 13 months), with first-party API traffic dominated by automation throughout (~70% Jan 2026) — consistent with the model's claim that engagement-optimized substitution (G3) is structurally favored over time, but slow.
...
The calculator-analogue baseline (decades of classroom calculator use without long-run cognitive decline) anchors the lower tail at λ = 0. Plausible λ at realistic offloading rates: roughly 0.02–0.10/year. The model's default λ = 0.06/year sits at the lower edge of this range.

**Verdict — bounded from below.** Calculator-analogue ruled out. Magnitude not pinned. The 2+ year longitudinal study with periodic capacity assessment that would actually fit λ does not yet exist. The model's λ = 0.06 is defensible as a working anchor; trajectory-tab predictions over 5–10 year horizons remain sensitive to whether λ is closer to 0.02 or 0.10.
...
Without the raw 300k-message dataset, only the qualitative shape is fittable. Calibrating ψ_R such that 60 minutes above threshold in a thin baseline (δ_R = 0.4) produces ΔM_rel ≈ −0.10 (meaningful but not catastrophic) gives ψ_R ≈ 0.0028 per minute — within rounding of the model's default 0.003. β_R unchanged at 0.001.
...
**Test.** BCG-Randazzo three-mode distribution (HBS WP 26-036): self-automator share **27%** at inferred f · ρ ≈ 0.06 (one or two interactions, abdicated co-creation, 44% accept output with zero modification, no skill development); cyborg + centaur **73%** at f · ρ-weighted centroid ≈ 0.49 (full-workflow integration, retained verification, measured upskilling). The midpoint between these centroids is **τ ≈ 0.25** — within 0.05 of the model default.
...
**Test.** Pool the per-task productivity studies into a per-domain α distribution. Implied α = treatment effect / (1 − s) at gate-open, with assumed-average s per study population.

| Domain | n studies | α median | α range | spread | Sources |
|---|---|---|---|---|---|
| Coding | 2 | 0.74 | 0.47 – 1.01 | 2.2× | Cui et al. 2024 (+26.08% weekly tasks); Peng et al. 2023 (+55.8% on JS HTTP-server) |
| Consulting | 2 | 0.52 | 0.24 – 0.80 | 3.3× | Dell'Acqua BCG 2023 (+12.2% productivity / +40% quality on the same tasks) |
| Writing | 2 | 0.485 | 0.30 – 0.67 | 2.2× | Noy & Zhang 2023 (−40% time / +18% quality) |
| Customer service | 2 | 0.355 | 0.28 – 0.43 | 1.5× | Brynjolfsson-Li-Raymond QJE 2025 (+14% overall / +34% novice) |
| Realized economy-level | 2 | 0.055 | 0.04 – 0.07 | 1.8× | Humlum-Vestergaard NBER 33777 (≤2% earnings; 3% self-reported time savings) |

(The two Bastani GPT-Base / GPT-Tutor education anchors land at α ≈ 1.25 median, but those are *during-practice assisted-performance* lifts rather than the per-task gain the model's α represents — they're shown separately in the panel above and excluded from the per-domain summary.)

Across the five per-task domains, α ranges from 0.24 to 1.01 — a **4.2× spread**. Median α = 0.45; the model's default α = 0.40 sits at the **37th percentile** — a lower-middle anchor that under-represents coding and writing. The realized-economy-level anchor (Humlum-Vestergaard ≤ 2% earnings, 3% self-reported time savings) is roughly 1/10th of the median per-task α — the J-curve gap between per-task gains and aggregate-economy effects.
