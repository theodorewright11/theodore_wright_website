...
**Cross-domain α heterogeneity.** The model's α = 0.40 is a midpoint compromise across studies that span ~4× in measured productivity gain. Brynjolfsson-Li-Raymond customer service +14% overall / +34% novice maps roughly to α ≈ 0.40. BCG consulting (Dell'Acqua-McFowland-Mollick) +12% productivity / +40% quality inside the frontier — same order of magnitude. **But coding domains show much larger gains.** Cui-Demirer-Jaffe-Musolff-Peng-Salz field experiments with software developers using Copilot found ~26% increase in pull-requests-merged-per-week (α effective ≈ 0.6–0.8 at typical developer s-values). Peng-Kalliamvakou-Cihon-Demirer's GitHub Copilot controlled experiment found 55% faster task completion (α effective ≈ 1.0+ for the specific tasks tested). The α = 0.40 calibration under-estimates ΔV_prod for coding work by roughly 2× and over-estimates it for customer-service-class tasks. Users in highly AI-suited domains (coding, writing, image generation, data analysis) should mentally adjust α up; users in domains where AI is awkward (in-person therapy, embodied physical work, deep relational work) should adjust α down. Stage 4 should fit α per major task category rather than treating it as a scalar — see Q6 in §9.
