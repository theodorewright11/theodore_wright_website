...
refinementLog:
  - pass: 8
    date: '2026-04-29'
    passes: ['internal consistency check', 'completeness']
    why: 'Two small but real notational issues from a final fresh-eyes scan. (1) §10 tech-utilization-architecture connection said "the current model''s λ parameter is the connection point" — but the model has *two* parameters named λ (the atrophy speed in ρ(t) = ρ₀·exp(−λ·u·t) and λ_M, the competence-erosion coefficient in ΔM_comp). Context disambiguates here in favor of atrophy-speed, but a reader scanning §10 directly would have to backtrack. Bare "λ" reference is sloppy when two λs exist. (2) §11 glossary had entries for every other model constant (α, η_trap, λ_M, ψ_R, β_R, d_safe, τ) but not σ — the logistic-gate transition width. σ is defined inline in §3.1 but absent from the glossary, breaking the otherwise complete reference list. After this pass, the model is at the genuine diminishing-returns threshold and ready for handoff to Stage 4.'
    changes:
      - '§10 tech-utilization-architecture connection: clarified bare "λ" as "λ (atrophy speed)" with parenthetical, removing the two-λs ambiguity'
      - '§11 glossary: added σ entry — "logistic-gate transition width; σ = 0.06 makes the gate transition smooth (10–90% transition over an f·ρ range of about 0.27)"'
  - pass: 7
    date: '2026-04-29'
    passes: ['error check']
    why: 'Recomputed the §3.5 cross-domain α numbers by hand and found four numerical inaccuracies. The §3.5 paragraph is the doc''s quantitative anchor for the "α varies across domains" claim, so the per-study α values need to be precise. Specifically: (1) Brynjolfsson "+14% overall / +34% novice maps roughly to α ≈ 0.40" is wrong — +14% overall with average s≈0.5 gives α=0.28; +34% novice with s≈0.2 gives α=0.43. The model''s α=0.40 picks up the *novice* anchor, not "roughly" both. (2) BCG "+12% productivity / +40% quality... same order of magnitude" elides a real ~3× spread — +12% gives α=0.24, +40% gives α=0.80. They''re each within 2× of 0.40 but not the "same order of magnitude" in any precise sense. (3) Cui "~26% PR throughput (α effective ≈ 0.6–0.8)" was inflated; 26% with s≈0.4–0.5 gives α≈0.5–0.65. (4) Peng "55% faster task completion (α effective ≈ 1.0+)" was correct but underspecified — 55% with s≈0.4–0.5 gives α≈1.1–1.4. Total span is α ≈ 0.24 to 1.4, which is ~6× not "~4×" as I claimed in §3.5 and C4. Tightening the per-study anchors and the headline span figure.'
    changes:
      - '§3.5: rewrote per-study α calculations with explicit (1−s) translations. Brynjolfsson now correctly characterised as a two-anchor study (overall α≈0.28 at s≈0.5; novice α≈0.43 at s≈0.2). BCG now correctly characterised as a two-outcome study (productivity α≈0.24, quality α≈0.80, ~3× spread within one study). Cui corrected from 0.6–0.8 → 0.5–0.65. Peng tightened from "1.0+" → "1.0–1.4." The model''s α=0.40 honestly placed: it picks up the Brynjolfsson-novice end of the range, slightly above BCG-productivity, well below BCG-quality and the coding studies'
      - '§8 C4 + §3.5: corrected the headline span figure from "α varies ~4× across measured domains" to "α varies ~6× across measured domains (0.24 to 1.4)" — the precise number is the actual range across the corrected per-study anchors, and "5–6×" is the honest characterisation rather than the rounded "4×"'
      - 'PRD pass-4 decisions log entry: corrected the same "~4× variation" claim to match'
  - pass: 6
    date: '2026-04-29'
    passes: ['internal consistency check', 'adversarial + steelman']
    why: 'Three issues from another fresh-eyes read. (1) TLDR para 3 still claimed "the topology''s three reframer mechanisms (G3 substitution, G4 offloading, G7 telic exhaustion) all admit clean closed-form representations" — but pass 5 fixed exactly this overclaim in §1, leaving the TLDR contradicting the body. The TLDR is the most-read section; an internal contradiction there is more visible than anywhere else in the doc. (2) §10 connections section''s opening sentence said "five sibling topics in the planned-topics list and one active topic," but the list contains BOTH Technology-Utilization-Architecture (active) and Human-Psych-Variation (active) — two active topics, not one. Mis-count of own connections. (3) §7 adversarial section had four objections (philosophical / mechanism / functional form / representational) but didn''t engage what is arguably the strongest meta-objection: the model''s structure systematically rewards conscientiousness — high f, high ρ, high B, low d are all "discipline pays off" parameters, and the model has no representation of cases where disciplined users still suffer because their domain is structurally compromised. Distinct from D3 (productivity-only optimization, which is about IGNORING relational/meaning channels) — the moralistic-discipline-frame objection is about the channels the model DOES include all leaning the same direction. Worth engaging head-on.'
    changes:
      - 'TLDR para 3: rewrote G3-claim to match §1''s pass-5 fix — "the topology''s two mechanistic reframers (G4 offloading, G7 telic exhaustion) admit clean closed-form representations; G3 (engagement-optimized substitution) is encoded only as a parameter source for ψ_R rather than as a represented mechanism, so re-calibrating ψ_R covers G3-shifts but the model''s structure does not change"'
      - '§10 intro: corrected count — "five sibling topics in the planned-topics list and one active topic" → "five sibling topics in the planned-topics list and two active topics" (Technology-Utilization-Architecture AND Human-Psych-Variation are both active)'
      - '§7: added Objection 5 — Moralistic discipline frame. The model''s structure rewards conscientiousness on every channel (high f / high ρ / high B / low d). This may match empirical reality (disciplined users do outperform in BCG, Brynjolfsson, Cui, Randazzo) but it also smuggles in the productivity-culture moral hierarchy. Steelman: even granting the equations, the model has no representation of "your discipline doesn''t matter because your domain is doomed" — there is no parameter that says structural disruption swamps personal effort. Response: partially conceded — the model rewards discipline conditionally on labor-market access (G5 scope limit) and AI capability regime (a parameter); when a → 1 on the user''s tasks, ΔV_prod → 0 even for disciplined users, and the apprenticeship-break scope limit is exactly the case where personal effort does not save you. The honest framing is: the model says "disciplined response matters within a labor-market and capability regime that you do not control" — not "disciplined response saves you from anything"'
  - pass: 5
    date: '2026-04-29'
    passes: ['fresh-eyes audit', 'error check']
    why: 'Three issues from a cold read with by-hand numerical verification. (1) §2.5 trajectory example was numerically wrong: claimed "under λ = 0.06 the same person has ρ(10) ≈ 0.36 ... gate closes from g ≈ 0.86 to g ≈ 0.12" but actual computation gives ρ(10) = 0.8·exp(−0.06·0.6·10) ≈ 0.56 and g(0.5, 0.56) ≈ 0.42. The "drifts from positive into firmly negative territory" framing was overclaiming — at λ = 0.06 (the doc''s stated "heavy cumulative regime"), the trajectory is moderate over 10 years, not dramatic. To get ρ(10) ≈ 0.36 with u = 0.6 you would need λ ≈ 0.13, more than double the cumulative-regime anchor. The cumulative-atrophy story is a slow process even in its strong form (half-life 19 years at λ = 0.06), and the prose was inflating its 10-year visibility. (2) §1 listed G3 (engagement-optimized substitution) as one of three reframer mechanisms that "admit clean closed-form representations," but G3 is not actually represented mechanistically in the model — it informs the calibration of ψ_R (the dose-response slope above d_safe) but the model has no machinery for the engagement-optimization mechanism itself. If G3 weakened (regulation, design changes), ψ_R would re-calibrate but the model''s structure would not change. G3 is a parameter source, not a represented mechanism. Softened the claim. (3) "How to read this stage" §3 referenced "the Brynjolfsson-Chandar-Chen entry-level worker" as one of the dashboard presets, but no such preset exists — Brynjolfsson-Chandar-Chen is the apprenticeship-break paper, now in §5 as scope-limit G5, not as a preset. The asymmetric-exploiter preset is the Brynjolfsson-Li-Raymond novice-skill-compression archetype (a different paper). Misattribution; corrected.'
    changes:
      - '§2.5: replaced wrong numerical example. Was: "ρ(10) ≈ 0.36, the gate closes from g ≈ 0.86 to g ≈ 0.12, and ΔNet drifts from moderate-positive into firmly negative territory." Now: "ρ(10) ≈ 0.56, the gate closes from g ≈ 0.84 to g ≈ 0.42, and ΔNet drifts measurably more negative as ΔV_prod compresses and ΔV_trap grows — drift magnitude over 10 years on the order of −0.15 ΔNet under default identity parameters; meaningful but not catastrophic, cumulative-atrophy is a slow process even in its strong regime"'
      - '§1: softened the closed-form-representation claim. G3 is correctly characterised as a parameter source (informing ψ_R calibration) rather than a represented mechanism. The G4 / G7 representations remain correctly characterised as mechanistic — G4 lives in ρ and the gate, G7 lives in the telic-absorption form'
      - 'How to read this stage §3: replaced "the Brynjolfsson-Chandar-Chen entry-level worker" with "the Brynjolfsson-Li-Raymond novice-skill-compression archetype" (which is what the asymmetric-exploiter preset actually instantiates); also added "the OpenAI-MIT high-dose companion-app user" for the heavy-companion preset which had no lit-review attribution'
  - pass: 4
    date: '2026-04-29'
    passes: ['cross-context verification', 'crux identification', 'adversarial + steelman']
    why: 'Three issues that hadn''t been examined yet. (1) C4 (α task-invariance) was asserted as a structural claim but the cross-domain evidence shows ~4× variation: Brynjolfsson-Li-Raymond customer service +14%/+34% (novice), BCG consulting 12%/40%, Cui-Demirer-Jaffe-Musolff-Peng-Salz Copilot/coding ~26%, Peng-Kalliamvakou-Cihon-Demirer GitHub Copilot controlled experiment ~55% faster task completion. The model''s α = 0.40 is a midpoint that under-estimates ΔV_prod for coding domains and over-estimates it for customer service. C4 as written claimed cross-task constancy when the data shows heterogeneity — needed honest tightening, not just acknowledgement. (2) Channel additivity was a hidden load-bearing assumption never named. The model writes ΔNet as a sum of six independent channels, but ΔM_comp (work-meaning erosion) and ΔM_rel (relational depletion) plausibly interact — a user with depleted work-meaning may respond differently to relational dose. Adding C6 (channel additivity / no interaction terms) makes this assumption explicit and testable. (3) Time-horizon was unspecified throughout. ΔV_prod is fast (productivity gain this year); ΔM_telic and ΔM_comp build over years; relational dose-response is intermediate. Adding them on a single ΔNet axis assumes commensurability across timescales. The trajectory tab partially addresses this for ρ but the static dashboard''s ΔNet has an implicit horizon. Added a horizon-spec note in §1.'
    changes:
      - '§3.5 (Productivity gain): added cross-context α paragraph documenting the ~4× spread (Brynjolfsson +14% / BCG 12% / Cui 26% / Peng 55%) with the implication that α = 0.40 is a midpoint that mis-calibrates per-domain — users on coding tasks should mentally bump α up; users on customer service should bump it down'
      - '§8 cruxes: rewrote C4 from "α is roughly constant across task types" to the truer "α varies ~4× across measured domains; the model''s scalar α is a midpoint compromise, and per-domain α should be substituted when it can be estimated. Stage 4 should fit α per major task category rather than treating it as a single constant"'
      - '§8 cruxes: added C6 (channel additivity / no interaction terms). The model''s six-channel sum assumes the channels are independent given parameters; if ΔM_comp moderates ΔM_rel (or vice versa), the additive form misrepresents the joint loss. Falsification window: paired survey data on work-meaning and relational outcomes for AI-heavy and AI-light users; a significant interaction term flips C6'
      - '§1 (What''s being formalised): added a horizon-spec paragraph stating that ΔV / ΔM should be read as comparative-statics changes over a "medium" horizon (~1–5 years) where the fast / slow channels meaningfully overlap; the trajectory tab generalises this to multi-year drift. Static-dashboard ΔNet at horizons shorter than ~1 year overweights ΔV; at horizons much longer than ~5 years the trajectory tab is the better lens'
      - 'Added Stage-4 fitting target Q6 (per-domain α) to §9'
  - pass: 3
    date: '2026-04-29'
