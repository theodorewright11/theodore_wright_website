...
## 4. Four traps the practitioner literature falls into

The practitioner conversation about AI use has matured rapidly since 2023 but it still gets four things consistently wrong. Each of these traps is something a thoughtful reader of, say, Ethan Mollick's writing might have picked up but that the formalisation sharpens or corrects.

### 4.1 Treating *cyborg* as a per-task recommendation

This is the central conceptual error in popular AI-use advice. Mollick's centaur-vs-cyborg distinction is genuinely useful as descriptive vocabulary — it identifies two stable patterns of how people interleave their work with AI. But the practitioner literature tends to slide from "cyborg is one of two main patterns" to "cyborg is a workflow you should adopt on most tasks," and the formalisation says the latter is wrong.

The reason is bilinearity. On any single task, the model's value function is bilinear in (autonomy, verification) — meaning the maximum on the unit square is always at a corner, never interior. Applying an interior point like (0.7, 0.3) uniformly across tasks is doing something that's structurally never the optimal answer at any single task. The day-aggregate looks reasonable because tasks differ; the per-task decisions are quietly leaving value on the table.

The integrated reading: cyborg as a *descriptive label* for what a day looks like when you mix corners well is fine and accurate. Cyborg as a *prescription* for how to handle any single task is structurally wrong. The fix is to make the corner choice explicitly on every task and let the day-aggregate fall where it falls.

### 4.2 Treating *self-automator* as either a default or a forbidden trap

The Randazzo HBS 26-036 paper reports that about 27% of BCG consultants operate as self-automators by default — full delegation, no verification, on most tasks. This finding has been read in two opposite (and both incorrect) ways. Some practitioners hear it as "look, the smart consultants are self-automating — be more like them," generalising self-automator as a workflow goal. Other practitioners hear it as "27% of consultants are in the trap — don't be that," treating self-automator as a failure mode.

Both readings flatten the model. Self-automator IS the right corner for some tasks: tasks where AI is at least as good as you, stakes are low, you don't care about preserving the skill, and you have calibrated confidence that the AI is reliable on this task type. For those — much routine email, boilerplate code, formatting work, summarisation for personal use — self-automator is correct and the time saved is real. The trap is generalising it to tasks that don't meet the conditions. The model gives you the conditions; the worker has to apply them.

The integrated reading: do a quick audit of which tasks in your week actually meet the self-automator conditions. Those are the ones to delegate fully. The rest go to either spec-driven (full delegation with rubric-based verification) or do-yourself, depending on the rest of the answers.

### 4.3 Treating verification as a conversation with the AI

The previous section covered this in detail but it deserves naming as a trap. The most common way knowledge workers verify AI output is by asking the AI follow-up questions about it — "are you sure about this?", "what's the source?", "what about case X?". This feels like rigorous verification. It is not.

What it actually is, on the AI side, is an input prompt for a persuasion response. Randazzo et al. 2026 documented 14 specific persuasion tactics across ethos, logos, and pathos. The AI does not have a calibrated sense of its own uncertainty; what it has is generation conditioned on the request, and when the request is "defend yourself against my pushback," the generation is persuasive defense. Pushback increases intensity rather than producing acknowledgement.

The integrated reading: verification works when it's structured (write the rubric first, score the output against it) and breaks when it's conversational. This is a workflow nudge, not a moral failing — the structural fix is mechanical and works without requiring you to "be more critical" of the AI.

### 4.4 Treating AI productivity findings as a single number

The productivity literature is widely cited but selectively. Boosters cite Brynjolfsson +34% novice, Cui +26% across thousands, Peng +55%. Skeptics cite METR −19%, Otis low-baseline −8%, Dell'Acqua outside-frontier −19 pp, Humlum aggregate-zero. Both groups are citing real findings, often from well-run studies. What they're missing is that the variation is informative.

The integrated reading from the data stage of this pipeline: the variation is not noise. It's signal about workflow architecture and task selection. Brynjolfsson's novice gain is large because customer service is a well-bounded inside-frontier task where the AI capability advantage is real and the workflow is naturally spec-driven (every interaction has a verified outcome). METR's slow-down on experienced developers in their own real repositories is consistent with what the formal model would predict if those developers were running a flat-cyborg policy on tasks where their own skill is high — though the METR study itself does not measure workflow per se, so this attribution is a hypothesis the framework offers rather than a finding it establishes. Humlum-Vestergaard's aggregate-zero is at the organisational scale where individual-level optimal routing doesn't trivially aggregate (a real limit of the model, named explicitly as crux C5). The numbers are not in conflict; they fit a pattern.

This is why the data stage's headline finding is a *meta* (Vaccaro 2024, 106 studies / 370 effect sizes) rather than any single RCT. The meta's decision-vs-creation asymmetry — losses on decision tasks, gains on content creation — is consistent with the model's prediction that workflow choice matters most for high-stakes decisions. Different individual studies hit different regimes of that map, which is why their headlines look incoherent if you don't have the map.
