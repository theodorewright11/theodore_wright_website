...
The practical implication is that for any individual knowledge worker, the leverage is overwhelmingly in *how you wire up the AI you have*, not in chasing the next frontier model. The gap between flat-cyborg routing on frontier capability and per-task routing on mid-tier capability is the cleanest interpretable version of this claim — and the build artifact's compare-strategies view materialises it on a representative day-mix.
...
The integrated reading from the data stage of this pipeline: the variation is not noise. It's signal about workflow architecture and task selection. Brynjolfsson's novice gain is large because customer service is a well-bounded inside-frontier task where the AI capability advantage is real and the workflow is naturally spec-driven (every interaction has a verified outcome). METR's slow-down is plausibly explained by experienced developers running flat-cyborg policies on tasks where their skill is already very high — the formal model would predict harm here. Humlum-Vestergaard's aggregate-zero is at the organisational scale where individual-level optimal routing doesn't trivially aggregate (a real limit of the model, named explicitly as crux C5). The numbers are not in conflict; they fit a pattern.
...
First, skill matters more over a longer horizon. The Bastani 17 pp drop is one session's atrophy under unfettered AI use. Sustained over years on a skill you care about, the cumulative effect is much larger. The skill-value parameter λ in the model is doing a lot of work here — for skills you intend to actively maintain, spec-driven is the corner even when the immediate-quality calculus would favour self-automator. The right time horizon to evaluate λ on is years, not days.
