---
title: 'Topology'
description: 'Dependency graph of the lit review. 65 nodes typed across nine classes (foundational assumptions / methods / empirical claims / logical necessities / generating mechanisms / synthesis / practitioner frameworks / open questions / distortions); six cruxes (A1, A2, A3, A6, L1, L3) and four variant views (Vulnerability / Flow / Minimal / Capability-regime). The genuinely novel structural move for this topic: encoding the practitioner ↔ academic operationalization bridge that runs 12–18 months out of phase.'
date: 2026-04-29
status: in-progress
refinementPass: 2
refinementLog:
...
Cannot be falsified — only ignored. Both are weight-5 logical-necessity nodes (the L class).
...
Highlights the six cruxes (A1, A2, A3, A6 foundational; L1, L3 logical guardrails) plus the weight-5 nodes downstream. If any crux flips, propagation is concentrated through this subgraph. Useful for stress-testing: pick a crux, imagine it inverts, and trace the consequences through the highlighted subgraph.
...
Which nodes go stale if frontier capability jumps? The central worry the lit review's adversarial section names. Of the 65 nodes, 34 sort into one of three regime-fragility classes; the rest (methods, supporting empirical findings, distortions) are regime-orthogonal. The classification:
