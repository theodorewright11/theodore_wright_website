...
**Response.** Conceded as the most consequential live risk (D1 in §10). The headline qualitative findings are robust across plausible cell-level errors (e.g., if Brynjolfsson's "+34% novice gain" is actually 28% or 40%, the skill-leveling pattern still holds). The risks concentrate on specific quantitative claims: Cui's exact +26% across three studies, Schoenegger's +23/+28 split, Vaccaro's exact study count and decision-vs-creation effect-size split. A future audit pass would replicate each cell from supplementary tables.

### Objection 3 — The model's defaults survive only in the loose sense of "not strongly contradicted"

ε default 0.15 is now bounded below qualitatively but not pinpointable. β default 0.05 sits inside a 4× bracket. φ default 0.30 is wrong by 5× in the regime where it was tested. The "supported" verdicts mask that the model passes a much weaker bar than "well-calibrated against data."

**Steelman.** Conceded. Pass 1 over-claimed cleanness; passes 2–4 each retracted false precision; the honest read is that the data doesn't strongly disconfirm the model, but it doesn't tightly calibrate it either.

**Response.** This is the right read of the four-pass progression and is now explicit in the TLDR's verdict tally (one strong qualitative; one bracketed direction-and-shape; three structural / consistency / convergent; one framed). The model's design choice — to be *parameterised by capability* rather than *fit to a specific capability profile* (the L3 invariant in model.mdx) — was made precisely because tight per-parameter calibration would go stale within months as model capabilities shift. The pipeline's job is not to pin the parameters; it's to confirm the model doesn't catastrophically fail against the current empirical record. By that standard the model survives, with three live cruxes (D1 cell correctness, D2 (c_H, c_AI) circularity, D5 Bastani uniformity) named for future audit.
...
**Response.** Conceded as a real structural threat. The honest extension is to make c_⋆ a function of (u, v, persuasion_resistance) rather than a fixed formula — held as a named scope-limit (§5) plus a model-stage future direction. The current pipeline's recommended use of (1, 1) for high-σ tasks should carry a "verify with structured rubric, not free dialogue" caveat to mitigate the persuasion-bombing channel. This is now in the §9 stage-5 handoff as an explicit dashboard-design constraint.
