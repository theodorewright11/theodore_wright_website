---
title: 'Topology'
description: 'Dependency graph of the lit review — typed nodes, typed edges, six crux nodes, weakest links, four variant views, an objections section, and a glossary. Updated through 2024-2025 literature on AM correction, within-family GWAS at scale, PGS portability, Scarr-Rowe, GEP replication, and missing-heritability closure.'
date: 2026-04-28
status: in-progress
refinementPass: 2
refinementLog:
  ...
  - pass: 2
    ...
    why: 'Pass-1 deferred web research because the parent harness initially seemed to block WebSearch. After the user pushed back, I confirmed WebSearch was available, ran 12 targeted searches across the topologys weakest links and open questions, and incorporated the 2024-2025 findings that actually shift the picture.'
    changes:
      - 'M9 (WGS) updated: Wang et al. 2025 Nature shows WGS in ~500k UK Biobank captures ~88% of pedigree-based narrow-sense heritability — "missing heritability" problem now substantially resolved for many traits'
      - 'E23 (PGS portability) reframed: Ding et al. 2023 Nature shows continuous decay along genetic-distance continuum (r=−0.95), not a discrete ancestry-group step function'
      - 'E25 (Scarr-Rowe) downgraded weight 3 → 2, status ~ → ✗: Figlio et al. 2024 found 39/42 PGI×SES interactions in education were NEGATIVE (compensatory direction); Scarr-Rowe is failing'
      - 'M4 (within-family designs) strengthened: Howe et al. 2022 extended to 178k siblings × 25 phenotypes — within-family approach now mature beyond just EA'
      - 'E14 (GEP) strengthened: Herlitz et al. 2025 systematic review (54 articles, 27 meta-analyses) confirmed the pattern across personality, verbal abilities, episodic memory, negative emotions'
      - 'O7 (AM-correction magnitude) updated: Ma, Wang, Border et al. 2024 introduced LAVA-Knock — first method to systematically reduce xAM-induced bias across 630 trait pairs'
      - 'G2 (passive rGE) extended: Nivard et al. 2024 found indirect genetic effects extend beyond the nuclear family to dynastic / extended-family / community processes'
      - 'O2 (Flynn reversal) updated: Pietschnig et al. 2024 Vienna cohort found the positive manifold itself may be weakening across cohorts — softly pressures A3 in a new way'
      - 'W1 (PGS×E generalization) updated: Allegrini 2020 + 2025 systematic reviews show the candidate-gene-failure extrapolation is partially holding the null'
      - 'W2 (Scarr-Rowe) substantially rewritten: from "context-dependent" to "failing — compensatory hypothesis is the better-supported pattern"'
      - 'W4 (AM-correction) updated: methods now emerging, field-wide answer likely in 2-3 years'
      - 'W5 (GEP mechanism) updated: pattern strengthened by 2025 review; mechanism still contested'
      - 'W6 (Flynn-reversal cause) updated with positive-manifold-weakening wrinkle'
      - 'TLDR para 3 (weakest links) substantially rewritten to reflect 2024-2026 state'
      - 'Crux 3 (A3) added 2024 wrinkle on positive-manifold weakening'
---
...
| **M9** | 4 | Whole-genome sequencing (rare-variant capture). 2025 follow-up (UK Biobank ~500k, Wang et al. 2025 *Nature*) captures ~88% of pedigree-based narrow-sense heritability across many traits (20% rare + 68% common variants). The "missing heritability" problem is now substantially resolved for many phenotypes | ✓ |
...
**2024 wrinkle**: Pietschnig et al. 2024 (Vienna 2005–2018 cohort) reported that the positive manifold may be *weakening* across recent cohorts — gains in some abilities aren't tracking gains in others. If this generalizes, A3 isn't refuted, but its strength becomes cohort-dependent. Worth watching; not yet load-bearing.
