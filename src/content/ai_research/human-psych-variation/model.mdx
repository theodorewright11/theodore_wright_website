...
The dashboard above stitches §3.1, §3.2, and §3.3 into one panel — sliders for trait class, age, `m`, `β_i/β_d`, and rare-variant share; outputs the variance decomposition and the three method-specific h² numbers. Three sanity-check anchors should hold under any sensible parameter choice:
...
These are the calibration targets. If the dashboard's stacked-bar moves these visibly off, something in the parameter mapping has drifted and needs rebuilding.
...
| `r_δ` | Cross-spouse correlation in additive genetic value; `≈ m · h²` at AM equilibrium for a hyperpolygenic trait (one-iteration approximation). |
...
| `V_A*` | Additive genetic variance at AM equilibrium; `V_A* = V_A / (1 − r_δ)` in the Crow–Felsenstein form. |
| `h²(t)` | Heritability as a function of age; saturating exponential in the Wilson form (§3.2). |
