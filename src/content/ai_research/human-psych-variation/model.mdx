...
**Inverse / partition problem** (what the dashboard does): given the *observed* heritability `h²_obs` (which is what twin and SNP studies report from real, AM-equilibrium populations), partition it into the random-mating-equivalent direct component `V(A_d)` and the AM-induced LD inflation `V(A_LD)`:
...
Worked anchors:
- **Educational attainment** with `m ≈ 0.4`, `h²_obs ≈ 0.40` (twin) → `r_δ ≈ 0.16`, `V(A_d) ≈ 0.34`, `V(A_LD) ≈ 0.06`. The dashboard's "cognitive" trait class with reduced parameters approximates this regime.
- **Height** with `m ≈ 0.25`, `h²_obs ≈ 0.85` → `r_δ ≈ 0.21`, `V(A_d) ≈ 0.67`, `V(A_LD) ≈ 0.18` — matches the 14–23% empirical "AM-inflated" share Border et al. and Yengo et al. report.
...
**β-level vs variance-level.** The ratio `β_i/β_d` quoted above is at the *regression-coefficient* level. Translating to a variance contribution requires squaring (and adding cross-terms):

```
V(A_i)              = β_i² · V(g) = (β_i/β_d)² · V(A_d)
2·Cov(A_d, A_i)     ≈ 2·(β_i/β_d) · V(A_d)        (when k → 1)
V_PGS,population − V_PGS,within  ≈ V(A_i) + 2·Cov(A_d, A_i)  =  V(A_d) · ((β_i/β_d)² + 2·(β_i/β_d))
```

Where `k` is the AM-induced correlation between transmitted and non-transmitted alleles within parents (typically close to 1).

For EA with `β_i/β_d ≈ 0.4` and within-family `V(A_d) ≈ 0.15`: `V(A_i) ≈ 0.024`, `2·Cov ≈ 0.12`, total genetic-nurture variance contribution ≈ 0.144. The dashboard displays *only* the pure `V(A_i) = (β_i/β_d)² · V(A_d)` piece (≈ 0.024 here) as a clean variance bucket; the cross-term `2·Cov(A_d, A_i)` is the leakage path that makes empirical twin h² overshoot the dashboard's "Twin h² (ACE)" output, and is acknowledged in the help text rather than allocated to a separate bar segment. The simplification keeps the variance budget bounded; the cost is that "indirect genetic share of phenotypic variance" via PGS comparisons (which sees both `V(A_i)` and `2·Cov`) appears larger than the dashboard's `V(A_i)` slice alone.

The relation `V(A_i) + 2·Cov(A_d, A_i) ≈ V_PGS,population − V_PGS,within-family` is approximate, not exact. The exact identity drops the `β_i²·k²` term and treats `k` as ~1; it's a serviceable order-of-magnitude relation for most psychological traits and is what turns "missing heritability after within-family correction" from a puzzle into a measurement.
