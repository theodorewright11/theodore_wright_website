...
const NODES: GraphNode[] = [
  ...
  { id: 'G4', type: 'G', weight: 5, domain: 'cross', label: 'Cognitive offloading via practice atrophy', detail: 'When you offload cognitive work to AI, you lose practice effects that maintain and build the skill. Mechanism is parallel for emotional offloading: outsourcing venting, sense-making, conflict rehearsal to AI may erode capacity for sustained attention to another\'s emotional state, tolerance of ambiguity in live conflict, and sitting with unresolved tension. This is the bridge node — same mechanism produces deskilling in work AND relational depth.', status: '✓' },
  ...
  { id: 'G6', type: 'G', weight: 4, domain: 'meaning', label: 'Identity foreclosure → catastrophic disruption response', detail: 'Marcia identity-formation framework + Petriglieri 2011, Caza et al. 2018: people with foreclosed single-strand professional identities have the worst outcomes when that identity is destabilized. AI shock as a forcing function on previously latent identity foreclosure. Drives S2 (identity diversification before destabilization).', status: '✓' },
  { id: 'G7', type: 'G', weight: 5, domain: 'meaning', label: 'Telic exhaustion: AI completes telic work in seconds', detail: 'Setiya: telic activities are aimed at completion; success consigns value to past (self-annihilating). When AI can complete telic projects in seconds, the share of meaning staked on telic completion shrinks — but the atelic share (durative — friendship, contemplation, parenting *as* parenting) is untouched. Generates F1 / S3.', status: '✓' },
  ...
  { id: 'G10', type: 'G', weight: 5, domain: 'meaning', label: 'Competence frustration (SDT)', detail: 'Self-Determination Theory: wellbeing depends on autonomy, competence, relatedness (+ Martela beneficence). AI most threatens competence — symbolically (AI can produce what I produce, faster) AND actually (cognitive offloading erodes underlying skill — see G4). Balwit-Cowen Free Press May 2025: impressed by AI\'s output AND humbled by how easily it does what used to feel uniquely valuable. Autonomy can be preserved; relatedness orthogonal except where AI substitutes for collaborators; beneficence robust.', status: '✓' },

  // Logical guardrails (L) — definitional / framing constraints
  { id: 'L1', type: 'L', weight: 5, domain: 'cross', label: 'Finding vs forecast vs interpretation', detail: 'Findings: Eloundou, Brynjolfsson-Li-Raymond, Dell\'Acqua-Mollick, Therabot RCT, OpenAI-MIT, Brynjolfsson-Chandar-Chen, Hui-Reshef-Zhou, Anthropic Economic Index. Forecasts: Aschenbrenner Situational Awareness, AI 2027, intelligence-explosion content. Interpretations: Cowen, Smith, Thompson, Setiya-applied-to-AI. Treat them differently. Ignoring the distinction = the most common analytical error in this discourse.', status: '✓' },
...
export default function AITransitionGraph() {
  ...
  const variantBlurb: Record<Variant, string> = {
    ...
    leverage: 'Decision-leverage view. Saturated = high individual leverage (you can act on it directly). Mid = medium (you can modify conditions). Faded = structural / outside individual control. The strategic recommendations (S nodes) cluster as the high-leverage core.',
  };
