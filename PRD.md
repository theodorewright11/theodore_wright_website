...
- `Theme` — `{ id, name, definition?, reasoning?, description? (legacy), parentIds: string[], color, order?, annotationLinks: { annotationId, role: 'core' | 'supporting' }[], includeCodeIds: string[], uncodedHighlights?, extraQuotes?: { text, source?, role? }[], rating?: ThemeRating, created_at }`. `extraQuotes` are non-anchored quotes kept from low-effort imports. A theme groups annotations (and optionally whole codes) into an interpretive cluster. The narrative is split into two Markdown fields — `definition` (what the theme captures + boundaries) and `reasoning` (the pattern observed) — kept separate so they can be embedded/exported independently for the AI-TA comparison work; the legacy single `description` is migrated into `definition` on load. `ThemeRating` = `{ grounding?, usefulness?, independence?, interpretationLevel?, prevalence? (each 1–5), notes? }`, but only **grounding / usefulness / interpretation level** are hand-rated in the UI — independence and prevalence are computed downstream (set-level filter / quote-derived) and not surfaced as rating controls.
