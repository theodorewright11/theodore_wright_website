...
**Core unit: the Run** — one AI thematic-analysis output tagged with the study's independent variables, following the run-naming convention **`{model}_{promptvariant}_v{version}_{datasource}_{rq}_{positionality}_run{n}`** (promptvariant ∈ `engineered_data` / `engineered_no_data` / `not_engineered_data`; datasource e.g. `20_als_comments` / `160_als_comments`; rq + positionality are single-token shorthands). Fields: **model / prompt variant / version / data source / rq / positionality / run # / notes** — free text with datalist autocomplete from prior runs. Pasting a full run name (or uploading a JSON file named to the convention) **autofills the fields** via `runName.ts` (`parseRunName` anchors on the known prompt-variant values, the `v{n}` token, and the trailing `run{n}`; `buildRunName` composes the display title everywhere — run cards, Rate column headers, selectors, exports). Each run anchors its quotes against a shared **corpus**.

**Three views** (top-bar toggle):

- **Runs** — setup. Upload a data CSV (`id,text` columns; aliases like `data`/`comment`/`body` accepted; headerless fallback) → each row becomes a corpus document `D1…Dn` in row order (the `[D{n}]` join key). Multiple corpora supported (e.g. the 20-comment refinement set vs the 160). Create a run: paste/upload the AI themes JSON (same format as qual-coding's "import AI ↓": `{ themes: [{ name, definition, reasoning, quotes: [{ text, source, role }] }], additional_text? }`, tolerant parsing shared with that dashboard) + fill/autofill the metadata fields. Quotes are anchored verbatim to corpus docs; non-anchored quotes are always kept, with possible-source suggestions (normalized-substring / word-overlap ≥ 0.6). Run cards show the composed name, metadata chips, rated-progress (`N/M rated`), per-axis means, inline metadata editing, delete, and a per-run **`export ↓`** that downloads `<run name>.themes-ratings.json` (same convention as qual-coding's themes-ratings export: per theme name/definition/reasoning/ratings/similarities/quotes + run metadata header).
- **Rate** — the rating workspace. **Up to 3 runs side by side, one column per run**, picked via removable run chips + a "+ show run…" select (persisted as `rateRunIds`). Each column has a sticky header: composed run name, metadata chips, corpus tag, and `N/M rated`. With a single run shown, a 1/2/3-column grid control applies to its theme cards instead. Display toggles: Definition / Reasoning / Quotes / Sources / Score hints. Each card carries six 1–5 **+ N/A** score strips — Grounding, Research question fit (evaluative); Interpretation level, AI prior novelty, Analytical novelty, Positionality influence (descriptive; the last marked **WIP** but fully functional) — plus a rating-notes field. A collapsible **rubric panel** shows every axis's question + all five level descriptions (accordion); with "Score hints" on, the selected score's full level text renders under the axis row (plus button tooltips). Clicking an anchored quote opens the source document in a modal with the span highlighted; a `✗` after a source tag marks a non-anchored quote and `D{n}?` links open fuzzy-match candidates. Each card ends with a **Theme similarity** section: a "+ compare with…" picker (same run's themes first, then other shown runs') creates a pairwise link rated 1–5/N-A with notes, removable with `×`; links are stored once per unordered pair and theme ids are global, so pairs span runs.
