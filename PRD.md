...
- **Insights** — analysis view over a selectable window (6 / 12 / 24 months / all). Headline stats (avg, median, highest/lowest month, this-month projected vs average); monthly spending trend with a 3-month rolling-average line and per-month budget reference; category composition over time (stacked, toggleable between Broad / Mid / Detailed levels) + window totals share; biggest month-over-month movers by category; net cash flow (diverging bars) + cumulative savings trajectory; budget adherence (% used per month); needs vs wants (with a **configurable** per-category needs set, defaulting to the Obligations broad); spending by account; top-10 biggest transactions; and a runway estimate from a device-local current-balance input. A partial first month (tracking started mid-month) is **always excluded from all monthly aggregations**. Insights preferences (window, exclude-partial, recurring set, balance) are device-local, not synced. Charts are token-themed CSS/SVG (no chart library) with a fixed categorical palette for broad categories.
...
- **2026-08-02**: **Favicon** replaced with a flat slanted **TW** monogram — solid black on a white tile, letters skewed −10°, T and W interlocked side by side. A 3D-extruded version was built and rejected: the depth turned to mush below 32px. Candidates + a 128/64/32/16 contact sheet are generated by `scripts/gen_favicons.mjs` into `public/favicons/`. The mark is three uniform-stroke paths — plain T crossbar, plain stem, plain W. Split-crossbar and tapered-letterform versions were built (they need filled polygons rather than strokes) and rejected as too fussy at favicon size. Each variant is `mark(color, weight, width, spread)`: `width` is how wide the mark sits in the 64-unit tile, `spread` slides the W right to open the letter spacing. Bounds are measured off the path data and scale/centring derived from them, so the knobs stay independent — change one without nudging the others back into place.
