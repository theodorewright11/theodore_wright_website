...
- **Dashboard** — month selector + headline cards (Spent / Budgeted / Remaining / Income / Net cash flow) + per-category breakdown grouped Broad → Mid → Detailed with per-row variance bars. Defaults to current month; navigation to past months uses the budget that was effective for that month.
...
- **Insights** — analysis view over a selectable window (6 / 12 / 24 months / all). Headline stats (avg, median, highest/lowest month, this-month projected vs average); monthly spending trend with a 3-month rolling-average line and per-month budget reference; category composition over time (stacked by broad) + window totals share; biggest month-over-month movers by category; net cash flow (diverging bars) + cumulative savings trajectory; budget adherence (% used per month); needs vs wants (with a **configurable** per-category needs set, defaulting to the Obligations broad); spending by account; top-10 biggest transactions; and a runway estimate from a device-local current-balance input. A partial first month (tracking started mid-month) is **excluded from all monthly aggregations by default**, toggleable. Insights preferences (window, exclude-partial, recurring set, balance) are device-local, not synced. Charts are token-themed CSS/SVG (no chart library) with a fixed categorical palette for broad categories.
...
- **Documents** — the active project's documents. The left sidebar (resizable) is **documents-only** — it lists documents grouped by folder (path-style nesting, e.g. `Interviews/Round 1`) with collapsible folder headers, and carries no links to the Codebook (codes are managed in the Codebook view / panel). Main pane shows the active document with title + metadata + body. The reading toolbar has four independent display controls: **Lines** (cosmetic — line numbers + the sentence/chars chunking control), **Codes** (a code-chip margin **column** beside the text), **Highlight** (code highlights on the text itself), and **Themes** (a themes margin **column** listing the doc's themes as clickable chips). Any column forces the body into aligned rows; Lines only governs numbering/chunking. The themes column shows only the themes present in the doc (each chip on the first line it appears); clicking a chip toggles that theme's highlight (multi-select, default none shown) — shown themes highlight their spans on the text in each theme's own color.

**Low-effort import mode** (per-project toggle in Info): a low-effort AI run often gives no `[D#]` sources and paraphrases rather than quotes verbatim, so most quotes can't be anchored to a document span. With this mode on, AI-theme import keeps those non-anchored quotes as the theme's `extraQuotes` (shown as plain text under the theme — in the detail card and the doc view — but not highlighted on the document; each also lists any **possible-match** documents found by normalized-substring / word-overlap scoring, so a paraphrase can suggest its source), and captures the file's top-level `additional_text` (intro/summary/frequency-table prose that doesn't map to a theme) into the project's editable `additionalText` box in Info. With the mode off, non-anchored quotes are reported as skipped and `additional_text` is ignored. `extraQuotes` also ride along in the themes+ratings JSON export's `supporting` list. A right-side **Notes** panel (toggle from the doc toolbar) opens a Markdown editor for personal commentary on the document (separate from the coded body; never appears in annotations).
...
- **Themes** — an analyst-level interpretive layer *above* the codes. Build a hierarchical theme tree; each theme has a Markdown narrative, a set of linked annotations (each toggled **core** vs **supporting**), optional uncoded-text highlights, an "auto-include codes" set, and a multi-axis rating. Themes are the synthesis step after line-by-line coding. An **"import AI ↓"** button ingests an AI thematic-analysis JSON (the inverse of the `corpus.md` export): each AI theme becomes a project theme, and each verbatim quote (tagged with its `[D{n}]` source) is located in the corresponding document and attached as an uncoded highlight. Quotes that don't match verbatim are reported, not dropped. This is the round-trip behind the AI-vs-analyst comparison workflow — export `corpus.md`, run it through an LLM prompt, import the result. The themes sidebar header carries **Doc** (document-view toggle), **Import AI ↓**, and **Clear all** (delete every theme in the project, with confirm). The theme detail card edits **Definition** and **Reasoning** as two sections. A **Doc** toggle in the sidebar header switches the pane to a read-as-a-document view (every theme as a depth-nested heading with definition, reasoning, supporting `[D{n}]`-tagged quotes, and the rating control inline so you can score while reading top-to-bottom).
- **Grading** — rubric-rating dashboard for assessing coding quality (built for comparing one project's coding against another, e.g. AI vs analyst). Three tabs: **Codes** (specificity 1–5), **Annotations** (accuracy 1–5, with a per-code drill-down), **Themes** (five hand-rated axes: grounding / research question fit / interpretation level / AI prior novelty / analytical novelty — prevalence is computed downstream; independence is replaced by typed theme-similarity links). The two novelty axes are scored against the no-data baseline output: **AI prior novelty** (is the subject matter present in no-data?) and **analytical novelty** (does it say something no-data doesn't / couldn't?). Ratings live on the entities themselves.
...
- **Info** — project-level metadata: editable name, a one-line description, and an `About` Markdown doc (Write/Read tabs) for project background, code-tree decisions, research questions, etc. Plus a **Low-effort import mode** toggle and an **Additional text** box (see below), a "Project at a glance" stat strip (docs / codes / annotations / metadata fields) and a created/updated date line.
...
**Multi-select projects**: the project picker dropdown gives each project (other than the active one) a checkbox to include it in the Explore aggregation. The active project is always included implicitly. The selection persists in `AppState.exploreProjectIds`. Each project row also has a **duplicate-documents** action (`⧉`) that clones just the documents (+ metadata schema + folders) into a fresh project with no codes / annotations / themes — a clean-slate twin of the corpus in the same document order (so `corpus.md`'s `[D{n}]` tags still line up). This is how you set up an AI-coding project to compare against your own: duplicate the analyst's project's documents, export `corpus.md`, run the LLM prompt, then `import AI ↓` the result into the twin, and compare the two in Explore.

**Data model**:

- `Project` — `{ version: 1, id, name, description?, about? (markdown), metadataSchema, documents, codes, annotations, themes?, folders?, lowEffort? (import mode), additionalText? (captured import prose), drive?, created_at, updated_at }`.
...
- `Theme` — `{ id, name, definition?, reasoning?, description? (legacy), parentIds: string[], color, order?, annotationLinks: { annotationId, role: 'core' | 'supporting' }[], includeCodeIds: string[], uncodedHighlights?, extraQuotes?: { text, source?, role?, possibleSources? }[], rating?: ThemeRating, created_at }`. `extraQuotes` are non-anchored quotes kept from low-effort imports; `possibleSources` lists documents that plausibly contain the quote. A theme groups annotations (and optionally whole codes) into an interpretive cluster. The narrative is split into two Markdown fields — `definition` (what the theme captures + boundaries) and `reasoning` (the pattern observed) — kept separate so they can be embedded/exported independently for the AI-TA comparison work; the legacy single `description` is migrated into `definition` on load. `ThemeRating` = `{ grounding?, usefulness?, independence?, interpretationLevel?, prevalence?, aiPriorNovelty?, analyticalNovelty? (each 1–5), notes? }`, but only five are hand-rated in the UI — **Grounding / Research question fit (the `usefulness` field) / Interpretation level / AI prior novelty / Analytical novelty** (grounding + RQ-fit evaluative; interpretation level + both novelty axes descriptive) — prevalence is computed downstream, and independence is captured via theme-similarity links rather than a 1–5 score.
- `ThemeRelation` — `{ from, to, type: 'related' | 'subsumes', similarity?: 1–5 }`, stored project-level in `Project.themeRelations`. Replaces the Independence rating: the theme detail card's **Theme similarity** section links a theme to others it overlaps — `related` (symmetric: says a similar thing — rate once, shows on both) or `subsumes` (directed: this theme captures the other; the other reads "subsumed by"), each with a 1–5 similarity. Exported as a `similarities` list per theme in the themes+ratings JSON.
...
- `<project>.themes-ratings.json` — compact themes-only artifact for analysis/embedding (SPUR): per theme `{ name, parent, definition, reasoning, ratings (grounding/researchQuestionFit/interpretationLevel/aiPriorNovelty/analyticalNovelty/notes), similarities ([{ other, type: related|subsumes|subsumed-by, similarity }]), quotes: [{ text, source ([D{n}] tag), role (null for non-anchored extra quotes), possibleSources? }] }`. `quotes` includes coded annotations, uncoded highlights, and low-effort `extraQuotes` (the latter may have a `null` role and carry `possibleSources` match candidates). Pairs with the raw AI-output JSON (which keeps the same separate fields) for the cross-condition comparison pipeline.
...
**Core unit: the Run** — one AI thematic-analysis output tagged with the study's independent variables, following the run-naming convention **`{model}_{promptvariant}-v{version}_{datasource}_{rq}_{positionality}_run{n}`**. Slots are underscore-separated; *within* a slot hyphens are used, so parsing is a plain split on `_`. Values: model `chatgpt5.5` / `claude` / `gemini` (`human-teddy` for the human reference set); promptvariant `engineered-data` / `engineered-no-data` / `not-engineered-data` (`na` for the human set) with the version attached by hyphen (`-v2`); datasource `20-als-comments` / `160-als-comments` / `na` (no-data runs); rq `rq1, rq2, …`; positionality `p1, p2, …`; run `run1, run2, …`. Fields: **model / prompt variant / version / data source / rq / positionality / run # / notes** — free text with datalist autocomplete from prior runs. Pasting a full run name (or uploading a JSON file named to the convention) **autofills the fields** via `runName.ts` (`parseRunName` splits on `_`, anchoring on the `-v{n}` slot and the trailing `run{n}`; `buildRunName` composes the display title everywhere — run cards, Rate column headers, selectors, exports). Each run matches its quotes against a shared **corpus**.

**Three views** (top-bar toggle):

- **Runs** — setup. Upload a data CSV (`id,text` columns; aliases like `data`/`comment`/`body` accepted; headerless fallback) → each row becomes a corpus document `D1…Dn` in row order (the `[D{n}]` join key). Multiple corpora supported (e.g. the 20-comment refinement set vs the 160). Create a run: paste/upload the AI themes JSON (same format as qual-coding's "import AI ↓": `{ themes: [{ name, definition, reasoning, quotes: [{ text, source, role }] }], additional_text? }`, tolerant parsing shared with that dashboard) + fill/autofill the metadata fields. Quote↔data matching is **tolerant**: exact `indexOf` first, then a canonical-space fallback (smart quotes → straight, dashes normalized, whitespace collapsed, case-insensitive) mapped back to exact original offsets — so curly-quote/line-break drift doesn't break matches. Unmatched quotes are always kept, with possible-source suggestions (word-overlap ≥ 0.6). Run cards show the composed name, metadata chips, `N/M Rated`, `n/m quotes in data`, per-axis means, plus **Rate / Export / Edit / Delete** (Export downloads `<run name>.themes-ratings.json`, same convention as qual-coding's themes-ratings export: per theme name/definition/reasoning/ratings/similarities/quotes + run metadata header). The Edit panel also has **Anchor against** (re-pick the corpus), **Re-match quotes** (re-runs matching over the run, preserving ratings) — for runs created before their data was uploaded — and **Replace JSON** (re-upload the run's themes file; themes matched by name keep their ids, ratings, and similarity links — for files that gained fields like `all_supporting_data`). Theme JSON may carry a per-theme `all_supporting_data: ["D3", …]` list — stored as `supportingData` and rendered on the Rate card as a `Supporting data (n):` line of clickable D-tags that open the doc modal; echoed in the per-run export.
- **Rate** — the rating workspace. **Up to 4 runs side by side, one column per run** (duplicates allowed, so a run can sit next to itself), picked via removable run chips + a "+ Show run…" select (persisted as `rateRunIds`). Each column has a sticky header: composed run name, metadata chips, corpus tag, and `N/M Rated`. A single shown run auto-lays its cards in two columns; no manual column control. Display toggles: Definition / Reasoning / Quotes / Sources / Score hints. Each card carries six 1–5 **+ N/A** score strips — Grounding, Research question fit (evaluative); Interpretation level, Novelty, Data contribution (vs the no-data baseline), Positionality contribution (vs the no-positionality baseline) (descriptive) — plus a rating-notes field. (Renamed 2026-07 from AI prior novelty / Analytical novelty / Positionality influence; old ratings migrate on load.) A collapsible **rubric panel** shows every axis's question + all five level descriptions (accordion); with "Score hints" on, a hint line renders under every axis row — the axis question when unrated, the selected score's full level text once rated. Quote lists are labeled `x/y in data`; clicking a matched quote opens the source document in a modal with the span highlighted; a `✗` after a source tag marks an unmatched quote and `D{n}?` links open fuzzy-match candidates. Each card ends with a **Theme similarity** section: a "+ Compare with…" picker (same run's themes first, then other shown runs') creates a pairwise link rated 1–5/N-A with notes, removable with `×`; links are stored once per unordered pair and theme ids are global, so pairs span runs.
- **Explore** — the analysis view. Chip filters per dimension (model / prompt variant / version / data source / rq / positionality) + per-axis score filters (any / 1–5 / N/A / unrated). A **mean-scores table** grouped by any dimension (means over numeric scores only; N/A counted separately). A theme table with all scores; clicking a theme jumps to it in Rate (adding its run to the shown set). Exports: `theme-grading.ratings.csv` (one row per theme with run metadata + all six scores — the analysis-ready artifact), `theme-grading.similarities.csv` (one row per pair), and a full-state JSON.
...
**Data model**: `Corpus { id, name, docs: [{ extId, text }] }` · `Run { id, corpusId, model, promptVariant, version, dataSource, rq, positionality, runN?, notes?, themes, additionalText? }` (legacy `condition`/`researchQuestion`/`repeat` fields migrate on load) · `RatedTheme { id, name, definition?, reasoning?, quotes: [{ text, source?, role?, anchor? (docIdx/start/end), possibleSources? }], rating }` · `SimilarityPair { themeA, themeB, similarity?, notes? }` (kept once added, even unscored; removed only explicitly).
...
- **2026-08-02**: **Favicon** replaced with a flat slanted **TW** monogram — solid black on a white tile, letters skewed −10°, T and W interlocked side by side. A 3D-extruded version was built and rejected: the depth turned to mush below 32px. Candidates + a 128/64/32/16 contact sheet are generated by `scripts/gen_favicons.mjs` into `public/favicons/`. The mark is **filled polygons, not uniform strokes** — a split needs two separate shapes and a taper needs the width to vary, neither of which a stroke can do. The W is built by `ribbon(points, widths, miter)`, a variable-width polyline offsetter; its `miter` cap is the pointiness knob (sharp Vs would otherwise mitre out to needles). The T is explicit trapezoids: `barHalves(gap, rise)` and `stem(bottomHalfWidth)`. Variants live in the `SHAPES` map (`a-split-full`, `b-split-bar`, `c-offset`, `d-pointier`, `e-blunter`); the live mark is `b-split-bar` — crossbar cut in two, stem one tapered slab. Bounds are measured off the path data and scale/centring derived from them, so reshaping a letter can't knock the mark off-centre.
- **2026-08-02**: `/research` stripped further — no `download all research` button on the index, and the detail page drops its status chip (`Finished` / `In Progress` / `Planned`), keeping just the year. Footer download labels sentence-cased and renamed (`Download` · `My research ↓` / `AI's research ↓` / `All ↓`); contact links and the theme selector capitalized to match.
