...
- **Grading** — rubric-rating dashboard for assessing coding quality (built for comparing one project's coding against another, e.g. AI vs analyst). Three tabs: **Codes** (specificity 1–5), **Annotations** (accuracy 1–5, with a per-code drill-down), **Themes** (four hand-rated axes: grounding / usefulness / interpretation level / novelty — prevalence is computed downstream; independence is replaced by typed theme relations). Ratings live on the entities themselves.
