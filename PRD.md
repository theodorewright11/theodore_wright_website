...
- **2026-08-02**: **Favicon** replaced with a flat slanted **TW** monogram — solid black on a white tile, letters skewed −10°, T and W interlocked side by side. A 3D-extruded version was built and rejected: the depth turned to mush below 32px. Candidates + a 128/64/32/16 contact sheet are generated by `scripts/gen_favicons.mjs` into `public/favicons/`. Letterform variants live in the `SHAPES` map (`a-plain`, `b-ligature`, `c-overhang`, `d-angled`, `e-split`, `f-dagger`, `g-flagged` — the T treatment varies, the W is shared); the live mark is `c-overhang`, whose crossbar runs well past the stem to the left. Bounds are measured off the path data and scale/centring derived from them, so reshaping a letter can't knock the mark off-centre, and stroke weight and mark size stay independent knobs.
