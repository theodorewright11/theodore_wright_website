...
- `Theme` — `{ id, name, definition?, reasoning?, description? (legacy), parentIds: string[], color, order?, annotationLinks: { annotationId, role: 'core' | 'supporting' }[], includeCodeIds: string[], uncodedHighlights?, extraQuotes?: { text, source?, role?, possibleSources? }[], similarThemeIds?: string[], rating?: ThemeRating, created_at }`. `extraQuotes` are non-anchored quotes kept from low-effort imports; `possibleSources` lists documents that plausibly contain the quote. A theme groups annotations (and optionally whole codes) into an interpretive cluster. The narrative is split into two Markdown fields — `definition` (what the theme captures + boundaries) and `reasoning` (the pattern observed) — kept separate so they can be embedded/exported independently for the AI-TA comparison work; the legacy single `description` is migrated into `definition` on load. `ThemeRating` = `{ grounding?, usefulness?, independence?, interpretationLevel?, prevalence?, novelty? (each 1–5), notes? }`, but only **grounding / usefulness / independence / interpretation level / novelty** are hand-rated in the UI (grounding + usefulness + independence evaluative; interpretation level + novelty descriptive) — only prevalence is computed downstream (quote-derived) and not surfaced as a rating control. The theme detail card also has a **Similar themes** picker (`Theme.similarThemeIds`) to mark which other themes this one overlaps — the qualitative companion to the Independence rating, exported as a `similar` list in the themes+ratings JSON.
